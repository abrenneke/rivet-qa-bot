  ...
  for await (const event of processor.events()) {
    if (event.type === 'partialOutput') {
      if (
        ...
        const currentOutput = coerceType(
          event.outputs['response' as PortId],
          'string'
        );

        const delta = currentOutput.slice(
          previousIndexes.get(event.node.id) ?? 0
        );
...
export function getSingleNodeStream(
  processor: GraphProcessor,
